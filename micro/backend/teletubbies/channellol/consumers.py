...
#             self.channel_name
#         )
#         friend_usernames = await self.friend_list(self.user)
...
#         # Arkadaş listesini çek ve kullanıcı adlarını al
#         friend_list = FriendList.objects.get(user=user).friends.all()
#         friend_usernames = sorted([friend.username for friend in friend_list])
#         return friend_usernames
#         # (Optional) Verify friend usernames (replace with your validation logic)
#         # valid_friends = [username for username in friend_usernames if User.objects.filter(username=username).exists()]
